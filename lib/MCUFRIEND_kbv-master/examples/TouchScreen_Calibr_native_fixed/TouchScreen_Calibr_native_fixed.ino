...
void readResistiveTouch(void)
{
    ...
    tp.x = map(tp.x, TS_MINX, TS_MAXX, tft.width(), 0);// magic function
    tp.y = map(tp.y, TS_MINY, TS_MAXY, tft.height(), 0);// magic function
    Serial.println("\t\ttp.x=" + String(tp.x) + ", tp.y=" + String(tp.y));
}
...
void loop()
{
    ...
    while (true) {readResistiveTouch();}    // tread water
}
